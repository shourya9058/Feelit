...
async function getSongs(folder) {
    currFolder = folder;
    try {
        let response = await fetch(`${folder}/`); // Removed leading slash for relative path
        ...
        for (let element of as) {
            if (element.href.endsWith(".mp3")) {
                // Adjust the song path for relative fetching
                let songName = decodeURI(element.href.split(`${folder}/`)[1]);
                songs.push(`${folder}/${songName}`);
            }
        }
        updateSongList();
    } catch (error) {
...
const playMusic = (track, pause = false) => {
    currentSong.src = `${currFolder}/${track}`; // Removed leading slash
...
const songMetadata = {
    "Princess.mp3": "Josh Woodward",
    "With No Goodbye.mp3": "Josh Woodward",
    "Lets Pretend.mp3": "Josh Woodward",
    "The Secret.mp3": "Anna Craig",
    "I Want You Close To Me.mp3": "JARA", 
    "Come On.mp3": "Doc Hartley",
    "Alive.mp3": "Song Writerz",
    "Red Light.mp3": "Egor Budenny",
};
